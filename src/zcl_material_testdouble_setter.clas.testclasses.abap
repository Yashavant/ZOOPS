...
CLASS lcl_material_details DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    DATA:
      f_cut  TYPE REF TO zcl_material_testdouble_setter,  "class under test
...
CLASS lcl_material_details IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    f_cut = NEW zcl_material_testdouble_setter( ).
    f_cut->setter_injection(
        im_doa = gd_doa
*        im_doa = NEW zcl_material_details( )
    ).
